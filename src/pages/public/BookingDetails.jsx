...
// ✅ Always display in UK time — handles BST/GMT automatically forever
...
export default function BookingDetails() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Safe: handle both Date object and ISO string (from localStorage rehydration)
      ...
      const bookingRef = await addDoc(collection(db, 'bookings'), {
        ...
        start_time: Timestamp.fromDate(startTime),   // ✅ always UTC in Firestore
        end_time: Timestamp.fromDate(endTime),         // ✅ always UTC in Firestore
      ...
      console.log('📧 Confirmation emails sent automatically via Cloud Function!');
            ...
            <CardContent className="p-4 sm:p-5 md:p-6 lg:p-8 pt-4 sm:pt-5 md:pt-6">
              <form onSubmit={onSubmit} className="space-y-4 sm:space-y-5 md:space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="victim_name" className="text-slate-700 font-semibold text-sm sm:text-base flex items-center gap-2">
                ...
                {/* Email */}
                <div className="space-y-2">
                  <Label htmlFor="victim_email" className="text-slate-700 font-semibold text-sm sm:text-base flex items-center gap-2">
                ...
                {/* Phone */}
                <div className="space-y-2">
                  <Label htmlFor="victim_phone" className="text-slate-700 font-semibold text-sm sm:text-base flex items-center gap-2">
                ...
                {/* Preferred Language */}
                <div className="space-y-2">
                  <Label htmlFor="preferred_language" className="text-slate-700 font-semibold text-sm sm:text-base flex items-center gap-2">
                ...
                {/* Notes */}
                <div className="space-y-2">
                  <Label htmlFor="victim_note" className="text-slate-700 font-semibold text-sm sm:text-base flex items-center gap-2">
                  ...
                  <Textarea
                    ...
                    id="victim_note"
                    placeholder="Is there anything specific you'd like the Advisers to know?"
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-indigo-200 rounded-lg sm:rounded-xl p-3 sm:p-4 shadow-sm">
                  <p className="text-xs sm:text-sm text-slate-700 leading-relaxed">
                    <strong className="text-indigo-700">Privacy Notice:</strong> Your information will be kept confidential and only shared with your assigned Adviser. We take your privacy seriously.
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2 sm:pt-4">
                  <Button
                    ...
                    className="w-full sm:flex-1 order-2 sm:order-1 border-2 border-slate-300 hover:bg-slate-100 hover:border-slate-400 text-slate-700 font-semibold h-12 sm:h-14 text-sm sm:text-base md:text-lg transition-all duration-300 shadow-sm hover:shadow-md"
                    disabled={submitting}
                  >
                    Back
                  </Button>
                  <Button
                    type="submit"
                    className="w-full sm:flex-1 order-1 sm:order-2 bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-600 hover:to-blue-700 text-white font-bold h-12 sm:h-14 text-sm sm:text-base md:text-lg shadow-md hover:shadow-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={submitting}
                  >
                    {submitting ? (
                      <>
